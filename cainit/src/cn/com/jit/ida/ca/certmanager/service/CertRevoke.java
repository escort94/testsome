...
import cn.com.jit.ida.ca.certmanager.service.request.CertRevokeRequest;
import cn.com.jit.ida.ca.certmanager.service.response.CertRevokeResponse;
...
public class CertRevoke extends CertService
{
  private Request request = null;
  private DebugLogger debugLogger = null;
  private OptLogger optLogger = null;
  private CertRevokeResponse response = null;
  ...
  public Response dealRequest(Request paramRequest)
  {
    try
    {
      if (LogManager.isDebug2())
        try
        {
          this.debugLogger.appendMsg_L2("cert revoke request request XML:\n" + new String(paramRequest.getData()));
        }
      ...
      try
      {
        int i = CertRevokeOpt.revokeCert(this.certRvkInfo);
        if (i == 0)
          throw new CertException("0601", "执行业务操作 没有证书被注销");
      }
      catch (IDAException localIDAException1)
      {
        this.debugLogger.appendMsg_L1("revoke cert operation error :" + localIDAException1.toString());
      ...
      if ((str2.equals("KMC")) && (!this.certInfo.getCertStatus().equals("Undown")))
      {
    ...
    catch (Exception localException2)
    {
      ...
      if (this.operator != null)
      {
        ...
        {
          this.response.setErr("8004" + localIDAException4.getErrCode());
          this.response.setMsg("证书注销服务 " + localIDAException4.getErrDesc());
      ...
      if ((localException2 instanceof IDAException))
      {
        ...
        this.response.setErr("8004" + localIDAException5.getErrCode());
        this.response.setMsg("证书注销服务 " + localIDAException5.getErrDesc());
        this.response.appendDetail(localIDAException5.getHistory());
      }
      else
      {
        this.response.setErr("80040706");
        this.response.setMsg("证书注销服务 其他错误 系统错误");
    ...
    finally
    {
      ...
      {
        if (LogManager.isDebug2())
          this.debugLogger.appendMsg_L2("cert revoke response XML:\n" + new String(this.response.getData()));
      }
    ...
    throws CertException
  {
    ...
    if ((str.equals("Revoke")) || (str.equals("UndownRevoke")))
    {
      this.debugLogger.appendMsg_L1("cert status check error : status = " + str);
      throw new CertException("0401", "证书状态检查 证书状态非法", new Exception("此证书已被注销，无法完成注销操作"));
    }
  }
  ...
  protected void validateCheck()
    throws CertException
  {
    CertRevokeRequest localCertRevokeRequest = null;
    try
    {
      localCertRevokeRequest = new CertRevokeRequest(this.request);
    }
    ...
    this.certRvkInfo.setCertSN(localCertRevokeRequest.getCertSN());
    this.sn = localCertRevokeRequest.getCertSN();
    if (this.sn == null)
    {
      this.certRvkInfo.setCertDN(localCertRevokeRequest.getCertDN());
      this.subject = localCertRevokeRequest.getCertDN();
      this.certRvkInfo.setCtmlName(localCertRevokeRequest.getCtmlName());
      this.ctmlName = localCertRevokeRequest.getCtmlName();
      ...
      try
      {
        this.certInfo = CertQueryOpt.queryPendingUnRVKCertInfo(this.subject, this.ctmlName);
      }
      catch (IDAException localIDAException3)
      {
    ...
    this.certRvkInfo.setReasonID(Integer.parseInt(localCertRevokeRequest.getRevokeID()));
    String str2 = localCertRevokeRequest.getRevokeDESC();
    if ((str2 != null) && (!str2.trim().equals("")) && (str2.trim().length() > 255))
      throw new CertException("0225", "数据有效性检查 注销证书原因描述的长度超长");
    this.certRvkInfo.setReasonDESC(localCertRevokeRequest.getRevokeDESC());
  }
 ...
 * Qualified Name:     cn.com.jit.ida.ca.certmanager.service.CertRevoke
