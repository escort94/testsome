...
public class CertReqDownOpt
{
  public static X509Cert certReqDown(CertInfo paramCertInfo)
    throws IDAException
  {
    ...
    if (localCTML.getCTMLStatus().equals("UNUSED"))
  ...
  public static X509Cert certReqDown(CertInfo paramCertInfo, ASN1Set paramASN1Set)
    throws IDAException
  {
    DBManager localDBManager = DBManager.getInstance();
    long l1 = localDBManager.getCertAccount();
  ...
  {
    DBManager localDBManager = DBManager.getInstance();
    int i = localDBManager.reqAndDownCert(paramCertInfo);
    if (i == -1)
      throw new IDAException("0606", "执行业务操作 数据库保存证书无效");
 ...
 * Qualified Name:     cn.com.jit.ida.ca.certmanager.service.operation.CertReqDownOpt
