...
public class Server implements Runnable {
	...
	public Server(NetConfig paramNetConfig, int paramInt1, int paramInt2,
			DealRequest paramDealRequest) throws NetException {
		...
		for (int i = 0; i < paramInt2; i++)
			new Thread(this.dealGroup, new DealReqPool(this, paramDealRequest))
					.start();
	...
	public boolean addDealThread() {
		new Thread(this.dealGroup, new DealReqPool(this, this.dealRequest))
				.start();
		return true;
	}
	...
	public void pauseServer() throws NetException {
		try {
			this.server.close();
	...
	public void stopServer() throws NetException {
		if (ServerState != 2)
			return;
		synchronized (new Integer(ServerState)) {
			ServerState = 1;
		...
		try {
			...
			throw new NetException("17", "停止服务器错误", e);
		}
		ServerState = 4;
		synchronized (DealReqPool.pool) {
	...
	public void starServer() throws NetException {
 ...
 * cn.com.jit.ida.net.server.Server JD-Core Version: 0.6.0
 */