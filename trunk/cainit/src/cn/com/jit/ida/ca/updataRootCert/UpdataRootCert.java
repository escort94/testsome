...
public class UpdataRootCert {
	...
	private boolean readInitInfo() {
		...
		try {
			int i = 0;
			Date localDate = this.globalConfig.getCAConfig().getRootCert()
					.getNotAfter();
			i = (int) ((localDate.getTime() - new Date().getTime()) / 86400000L);
			if (i <= 0)
				i = 1;
			i++;
			String str1 = ConfigTool.getInteger("请输入CA证书有效期，单位为天", 1825000, i,
					"天");
			if (str1 == null)
				return false;
			this.validity = Integer.decode(str1).intValue();
			if (this.globalConfig.getCryptoConfig().getDeviceID()
					.equalsIgnoreCase("JSJY05B_LIB")) {
				String str2 = ConfigTool.getInteger("请选择密钥对标识(>0)", 100, 1, "");
				if (str2 == null)
					return false;
				this.newKeyID = Integer.decode(str2).intValue();
				String str3 = this.globalConfig.getCryptoConfig()
						.getCommDeviceID();
	...
	public boolean run() throws UpdataRootException {
		try {
			this.globalConfig = GlobalConfig.getInstance();
		} catch (IDAException localIDAException1) {
			throw new UpdataRootException(localIDAException1.getErrCode(),
					localIDAException1.getErrDesc(), localIDAException1);
		}
		JCrypto localJCrypto = JCrypto.getInstance();
		try {
			this.session = localJCrypto.openSession(this.globalConfig
					.getCryptoConfig().getDeviceID());
		} catch (PKIException localPKIException1) {
			throw new UpdataRootException(localPKIException1.getErrCode(),
					localPKIException1.getErrDesc(), localPKIException1);
		}
		if (!readInitInfo())
			return false;
		try {
			this.oldKeyPair = new JKeyPair(this.globalConfig.getCAConfig()
					.getRootCert().getPublicKey(), this.globalConfig
					.getCAConfig().getPrivateKey());
		} catch (PKIException localPKIException2) {
			throw new UpdataRootException(localPKIException2.getErrCode(),
					localPKIException2.getErrDesc(), localPKIException2);
		}
		GenNewKeyPair();
		generateAllCert();
		saveCert(this.newCert, this.newKeyPair.getPrivateKey(), "Useing",
				this.globalConfig.getCryptoConfig().getDeviceID());
		saveCert(this.O_NCert, this.newKeyPair.getPrivateKey(), "Useing",
				this.globalConfig.getCryptoConfig().getDeviceID());
		saveCert(this.N_OCert, this.oldKeyPair.getPrivateKey(), "Useing",
				this.globalConfig.getCryptoConfig().getDeviceID());
		GenKeyStore();
		SavaRootCert();
		revokeCert();
		...
		try {
			...
			if ((localObject != null)
					&& (((String) localObject).equalsIgnoreCase("true")))
				i = 1;
		} catch (ConfigException localConfigException) {
			throw new UpdataRootException(localConfigException.getErrCode(),
		...
		if (i != 0) {
			...
			try {
				...
				throw new UpdataRootException(localIDAException2.getErrCode(),
			...
			try {
				...
				throw new UpdataRootException("0899", "系统错误 构造发布管理器失败",
						localException1);
			}
			try {
				((CAEntity) localObject).setCACert(this.newCert.getEncoded());
				((CAEntity) localObject).setCACertSN(this.newCert
						.getStringSerialNumber());
				((CAEntity) localObject)
						.setCASubject(this.newCert.getSubject());
			...
			try {
				...
				((CAEntity) localObject).setCACertSN(this.O_NCert
						.getStringSerialNumber());
				((CAEntity) localObject)
						.setCASubject(this.O_NCert.getSubject());
				localIssuanceManager.issue((BaseEntity) localObject);
			...
			try {
				...
				((CAEntity) localObject).setCACertSN(this.N_OCert
						.getStringSerialNumber());
				((CAEntity) localObject)
						.setCASubject(this.N_OCert.getSubject());
				localIssuanceManager.issue((BaseEntity) localObject);
	...
	void GenKeyStore() throws UpdataRootException {
		...
		try {
			...
			if (this.globalConfig.getCryptoConfig().getDeviceID()
					.equalsIgnoreCase("JSOFT_LIB")) {
				Key localKey = Parser.convertKey(this.newKeyPair
						.getPrivateKey());
				localKeyStore.setKeyEntry(this.globalConfig.getCAConfig()
						.getRootCert().getSubject(), localKey,
						this.globalConfig.getCryptoConfig().getRootCertPWD()
								.toCharArray(), arrayOfCertificate);
			} else {
				this.globalConfig.getCAConfig().setCASignKeyID(this.newKeyID);
				localKeyStore.setCertificateEntry(this.globalConfig
						.getCAConfig().getDN(), localCertificate);
			}
		} catch (Exception localException1) {
			throw new UpdataRootException("0803", "产生KeyStore错误",
					localException1);
		}
		FileOutputStream localFileOutputStream = null;
		try {
			localFileOutputStream = new FileOutputStream(new File(
					this.globalConfig.getCryptoConfig().getRootCert()));
			localKeyStore.store(localFileOutputStream, this.globalConfig
					.getCryptoConfig().getRootCertPWD().toCharArray());
		} catch (Exception localException2) {
			throw new UpdataRootException("0804", "产生KeyStore文件错误",
					localException2);
		} finally {
			try {
	...
	private void generateAllCert() throws UpdataRootException {
		...
		byte[] arrayOfByte2 = GenerateAcrossCert(str, this.newKeyPair
				.getPublicKey(), this.oldKeyPair);
		try {
		...
		byte[] arrayOfByte3 = GenerateAcrossCert(str, this.oldKeyPair
				.getPublicKey(), this.newKeyPair);
		try {
			this.N_OCert = new X509Cert(arrayOfByte3);
		} catch (PKIException localPKIException3) {
			throw new UpdataRootException(localPKIException3.getErrCode(),
					localPKIException3.getErrDesc(), localPKIException3);
		}
	}

	private byte[] GenerateNewCert() throws UpdataRootException {
		...
		try {
			...
			localConfigFromDB = new ConfigFromDB(localDBManager,
					"SigningCertExtension");
		} catch (IDAException localIDAException) {
			throw new UpdataRootException(localIDAException.getErrCode(),
		...
		try {
			...
			localX509CertGenerator.setSignatureAlg(localX509Cert
					.getSignatureAlgName());
			...
			return localX509CertGenerator.generateX509Cert(this.newKeyPair
					.getPrivateKey(), this.session);
		} catch (PKIException localPKIException) {
		}
		throw new UpdataRootException(localPKIException.getErrCode(),
				localPKIException.getErrDesc(), localPKIException);
	}

	private byte[] GenerateAcrossCert(String paramString, JKey paramJKey,
			JKeyPair paramJKeyPair) throws UpdataRootException {
		X509CertGenerator localX509CertGenerator = new X509CertGenerator();
		...
		try {
			...
			localX509CertGenerator.setIssuer(this.globalConfig.getCAConfig()
					.getRootCert().getX509NameSubject());
			localX509CertGenerator.setNotBefore(localDate);
			localX509CertGenerator.setNotAfter(this.globalConfig.getCAConfig()
					.getRootCert().getNotAfter());
			localX509CertGenerator.setPublicKey(paramJKey);
			localX509CertGenerator.setSignatureAlg(this.globalConfig
					.getCAConfig().getRootCert().getSignatureAlgName());
			AuthorityKeyIdentifierExt localAuthorityKeyIdentifierExt = new AuthorityKeyIdentifierExt(
					paramJKeyPair.getPublicKey());
			...
			return localX509CertGenerator.generateX509Cert(paramJKeyPair
					.getPrivateKey(), this.session);
		} catch (PKIException localPKIException) {
		}
		throw new UpdataRootException(localPKIException.getErrCode(),
	...
	private void GenNewKeyPair() throws UpdataRootException {
		Mechanism localMechanism;
		if (this.globalConfig.getCAConfig().getPrivateKey().getKeyType()
				.equalsIgnoreCase("RSA_Private")) {
			localMechanism = new Mechanism("RSA");
		} else if (this.globalConfig.getCAConfig().getPrivateKey().getKeyType()
				.equalsIgnoreCase("RSA_PrivateID")) {
		...
		try {
			int i = Parser.getRSAKeyLength(this.globalConfig.getCAConfig()
					.getRootCert().getPublicKey());
	...
	private void saveCert(X509Cert paramX509Cert, JKey paramJKey,
		...
		try {
			if (paramString2.equalsIgnoreCase("JSOFT_LIB")) {
				...
				localCACertInfo.setPrivateKey(ProtectConfig.getInstance()
						.Encrypto(String.valueOf(this.newKeyID)));
				this.globalConfig.getCAConfig().setCASignKeyID(this.newKeyID);
			} else {
		...
		try {
			...
			throw new UpdataRootException(localDBException.getErrCode(),
					localDBException.getErrDesc(), localDBException);
		}
	}

	private void SavaRootCert() throws UpdataRootException {
		...
		File localFile = new File(this.globalConfig.getCryptoConfig()
				.getRootCert());
		String str;
	...
	private void revokeCert() throws UpdataRootException {
	}

	private void addCertificatePolicy(X509CertGenerator paramX509CertGenerator,
			ParseXML paramParseXML) throws UpdataRootException {
		try {
			...
			if (paramParseXML.getString("CertificatePolicy_IsCritical")
	...
	private void addAuthorityKeyIdentifier(
			X509CertGenerator paramX509CertGenerator, ParseXML paramParseXML)
			throws UpdataRootException {
		try {
			String str = paramParseXML
					.getString("AuthorityKeyIdentifier_IsExist");
			if (!str.equalsIgnoreCase("true"))
				return;
			AuthorityKeyIdentifierExt localAuthorityKeyIdentifierExt = new AuthorityKeyIdentifierExt(
					this.newKeyPair.getPublicKey());
			if (paramParseXML.getString("AuthorityKeyIdentifier_IsCritical")
					.equalsIgnoreCase("true"))
	...
	private void addSubjectKeyIdentifier(
		...
		try {
			String str = paramParseXML
					.getString("SubjectKeyIdentifier_IsExist");
			if (str.equalsIgnoreCase("false"))
				return;
			SubjectKeyIdentifierExt localSubjectKeyIdentifierExt = new SubjectKeyIdentifierExt(
					this.newKeyPair.getPublicKey());
			if (paramParseXML.getString("SubjectKeyIdentifier_IsCritical")
					.equalsIgnoreCase("true"))
	...
	private void addBasicConstraints(X509CertGenerator paramX509CertGenerator,
		...
		try {
			...
			if (paramParseXML.getString("BasicConstraints_IsCritical")
					.equalsIgnoreCase("true"))
	...
	private void addCRLDistPointExt(X509CertGenerator paramX509CertGenerator,
			ParseXML paramParseXML) throws UpdataRootException {
		try {
			if (!paramParseXML.getString("CDP_IsExist")
					.equalsIgnoreCase("true"))
				return;
			Object localObject1 = null;
			...
			if (paramParseXML.getString("CDP_LDAP_URI_Publish")
					.equalsIgnoreCase("true")) {
			...
			if (paramParseXML.getString("CDP_IsCritical").equalsIgnoreCase(
					"true"))
	...
	private void addKeyUsageExt(X509CertGenerator paramX509CertGenerator,
		...
		try {
			...
			if (paramParseXML.getString("KeyUsage_IsCritical")
					.equalsIgnoreCase("true"))
 ...
 * cn.com.jit.ida.ca.updataRootCert.UpdataRootCert JD-Core Version: 0.6.0
 */